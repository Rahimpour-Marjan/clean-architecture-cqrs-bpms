    ...
    public class UserLogRepository : IUserLogRepository
    {
        ...
        public async Task<IList<UserLog>> FindAll()
        {
            return await _db.UserLogs
                            .Include(x => x.Creator).ThenInclude(x => x.Account)
                            .ToListAsync();
        }
        public async Task<Tuple<IList<UserLog>, int>> FindAll(int? userId, QueryFilter? queryFilter)
        {
            var query = _db.UserLogs.Where(x => userId == null || x.CreatorId == userId)
                            .Include(x => x.Creator).ThenInclude(x => x.Account).AsQueryable();
        ...
        {
            var userLog = await _db.UserLogs.Include(x => x.Creator).ThenInclude(x => x.Account).FirstOrDefaultAsync(a => a.Id == id);
            if (userLog != null)
                return userLog;
            return null;
        }
